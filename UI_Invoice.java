import java.awt.*;
import java.awt.event.*;
import javax.swing.*;

/**
 * Write a description of class UI_Invoice here.
 *
 * @author (your name)
 * @version (a version number or a date)
 */
public class UI_Invoice extends JFrame
{
    // instance variables - replace the example below with your own
    private JPanel jp1, jp2, jp3, jp4, jp5, jp6;
    private JLabel jl1, jl2, jl3, jl4, jl5, jl6, jl7, jl8;
    private JButton jb1, jb2;

    /**
     * Constructor for objects of class UI_Invoice
     */
    public static void main(Hotel h, Reservation reservation)
    {
        UI_Invoice window = new UI_Invoice(h, reservation);
        JFrame frame = new JFrame("Invoice");
    }

    /**
     * An example of a method - replace this comment with your own
     *
     * @param  y  a sample parameter for a method
     * @return    the sum of x and y
     */
    public UI_Invoice(Hotel h, Reservation reservation)
    {
        Guest guest= reservation.getGuest();
        Room room = reservation.getRoom();
        jl1 = new JLabel("Invoice #" + reservation.getReservationID());
        jl2 = new JLabel("Base Rate: " + room.getRate());
        jl3 = new JLabel("Night Stays: " + guest.getNights());
        jl4 = new JLabel("Guest: " + guest.getFullName());
        jl8 = new JLabel("Party Size: " + guest.getPartySize());
        jl5 = new JLabel(discountString(guest)+" Discount: " + reservation.getDiscount());
        jl6 = new JLabel("Total: " + room.getRate() + " * " +guest.getNights() + " * " +reservation.getDiscount() + " = " + room.getRate()*guest.getNights()*reservation.getDiscount());

        jb1 = new JButton("Back to main menu");
        jb2 = new JButton("Pay and reserve");

        jp1 = new JPanel();
        jp2 = new JPanel();
        jp3 = new JPanel();
        jp4 = new JPanel();
        jp5 = new JPanel();

        jp4.add(jb1);
        jp5.add(jb2);

        jp1.setLayout(new GridLayout(1,2));
        jp1.add(jp4);
        jp1.add(jp5);

        jp3.setLayout(new GridLayout(8,1));
        this.setLayout(new GridLayout(1,3));
        jp3.add(jl1);
        jp3.add(jl2);
        jp3.add(jl3);
        jp3.add(jl4);
        jp3.add(jl8);
        jp3.add(jl5);
        jp3.add(jl6);
        jp3.add(jp1);
        this.add(new JPanel());
        this.add(jp3);
        this.add(new JPanel());
        this.setSize(800,600);
        this.setLocation(300,300);
        this.setResizable(false);
        this.setVisible(true);
        this.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);

        jb1.addActionListener(new ActionListener(){
                @Override    
                public void actionPerformed(ActionEvent e){
                    int temp = JOptionPane.showConfirmDialog(null, "Are you sure you want to back to main menu?", "Warning", JOptionPane.YES_NO_OPTION);
                    if(temp == 0){
                        Menu.main(h);
                        dispose();
                    }
                }
            });

        jb2.addActionListener(new ActionListener(){
                @Override    
                public void actionPerformed(ActionEvent e){
                    h.addReservation(reservation);    
                    room.setAvailable(false);
                    JOptionPane.showMessageDialog(null, "Reservation Successful!", "Message", JOptionPane.PLAIN_MESSAGE);
                    Menu.main(h);
                    dispose();
                }
            }
        );
        this.addWindowListener(new WindowAdapter() {
                @Override
                public void windowClosing(WindowEvent we)
                { 
                    int temp = JOptionPane.showConfirmDialog(null, "Are you sure you want to back to main menu?", "Warning", JOptionPane.YES_NO_OPTION);
                    if(temp == 0){
                        Menu.main(h);
                        dispose();
                    }
                }
            });
    }

    private static String discountString(Guest guest){
        String str = "No";
        if ( guest.isGovernment() ) {
            str = "Government";
        }
        else if (guest.isMilitary() ) {
            str = "Military";
        }
        else if (guest.isMember() ) {
            str = "Member";
        }
        return str;
    }

}
